// la diega — INTERFAZ del sintetizador: lo que se ve y se toca.
// De arriba abajo: dos potes (onda y filtro), presets / efectos en pestañas,
// el teclado de una octava de do a do —con las negras, y Z / X para cambiar
// de octava— y el volumen. El osciloscopio se arrastra como un theremin.
// El motor de audio vive en synth.js; la ficha que lo envuelve, en about.js.
...
let raiz = null;             // la ficha abierta ahora mismo (para el teclado)
let tecladoPuesto = false;
let sueltaTeclaDoc = null;   // el listener de soltar tecla que hay puesto ahora
const oscAnim = { on: false, raf: null, phase: 0 };

export function synthEncendido() { return oscAnim.on; }
...
// el osciloscopio cruza la pantalla entera: va aparte porque en la ficha
// queda por encima del nombre, y el resto del sinte por debajo
export function renderScope() {
    return '<canvas class="oscab-scope" width="640" height="140"></canvas>';
}
...
export function renderSynth() {
    ...
    return bloque('<div class="oscab-pots">' + pot('wave', 'tipo de onda') + pot('filter', 'filtro') + '</div>') +
        bloque(
...
export function renderSynthModal() {
    return '<div class="oscab-modal hidden">' +
...
export function modalAbierto(ov) {
    const modal = ov && ov.querySelector('.oscab-modal');
    return !!modal && !modal.classList.contains('hidden');
}

export function cierraModal(ov) { closeModule(ov); }

// apagar del todo: para el dibujo y el motor de audio
export function stopSynth() { oscAnimStop(); }

export function olvidaFicha() { raiz = null; }
...
export function wireSynth(ov) {
    raiz = ov;
    if (!tecladoPuesto) { bindTeclado(); tecladoPuesto = true; }
...
/* ===== Teclado del ordenador: las blancas en A S D F G H J K, las negras
   en W E · T Y U, Z / X cambian de octava y el espacio mantiene la nota.
   El sinte solo esta encendido con la ficha abierta (al cerrarla se para el
   motor), asi que con mirar si hay motor basta para saber si toca sonar ===== */

const teclas = new Set();

function bindTeclado() {
    document.addEventListener('keydown', e => {
        if (!raiz || !synth.engineOn()) return;
        if (e.key === ' ') {
            e.preventDefault();
            if (!e.repeat) { synth.noteOn(SYNTH.note); pintaTeclas(raiz, SYNTH.note); }
            return;
        }
        ...
        if (k === 'z' || k === 'x') {
            e.preventDefault();
            if (!e.repeat) cambiaOctava(raiz, k === 'x' ? 1 : -1);
            return;
        }
        ...
        teclas.add(k);
        synth.noteOn(i);
        pintaTeclas(raiz, i);   // la tecla se enciende tambien desde el teclado
    });
    document.addEventListener('keyup', e => {
        if (!raiz || !synth.engineOn()) { teclas.clear(); return; }
        if (e.key === ' ') { synth.noteOff(); pintaTeclas(raiz); return; }
        if (!teclas.delete(e.key.toLowerCase())) return;
        if (!teclas.size) { synth.noteOff(); pintaTeclas(raiz); }
        else {
