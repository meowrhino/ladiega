...
- **about-sintetizador**: la ficha esconde un sinte de verdad ("encender el sinte"): **pads** con las 11 notas de la pentatónica (dedo, ratón o teclado A–Ñ), **presets** (nave · campana · coro · goma · chunda) que cambian todo de un clic, osciloscopio a pantalla completa que se toca como un theremin, tres potes (onda / nota / filtro), y cinco módulos —voces con detune y spread estéreo, chorus · flanger · reverb (pre-delay y dry/wet de crossfade) · arpegio— donde **el nombre enciende el efecto y los "···" abren sus ajustes** (en cristiano, con el nombre técnico debajo). Más el chip "drone" (siempre sonando ↔ solo al tocar), volumen y el chip "canción" que mutea/recupera el video de fondo.
